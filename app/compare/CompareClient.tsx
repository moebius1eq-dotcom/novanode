...
function CompareClientContent() {
  const searchParams = useSearchParams();
  const spots = spotsData.spots as WorkSpot[];
  
  // Get spots from URL params
  const spotIds = searchParams.get("spots")?.split(",").filter(Boolean) || [];
  const initialSelected = spots.filter(s => spotIds.includes(s.id));
  
  const [selectedSpots, setSelectedSpots] = useState<WorkSpot[]>(initialSelected);
  ...
  const updateUrl = (selection: WorkSpot[]) => {
    const params = new URLSearchParams();
    if (selection.length > 0) {
      params.set("spots", selection.map(s => s.id).join(","));
    }
...
export default function CompareClient({ searchParams }: { searchParams: Promise<{ spots?: string }> }) {
  return (
    <Suspense fallback={<LoadingState />}>
      <CompareClientContent />
    </Suspense>
  );
}
