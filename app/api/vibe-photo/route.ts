...
interface VibePhoto { spotId: string; url: string; createdAt: string; }
const dataPath = path.join(process.cwd(), "data", "vibe-photos.json");
const uploadDir = path.join(process.cwd(), "public", "uploads");

async function readAll(): Promise<VibePhoto[]> {
  try { return JSON.parse(await fs.readFile(dataPath, "utf8")) as VibePhoto[]; } catch { return []; }
}
...
export async function GET(request: NextRequest) {
  ...
  if (isSupabaseConfigured()) {
    ...
    const { data, error } = await supabase!
      ...
      .limit(1);
    if (!error && data) {
      return NextResponse.json({ items: data.map((d) => ({ url: d.url as string, createdAt: d.created_at as string })) });
    }
  }
...
export async function POST(request: NextRequest) {
  ...
  if (isRateLimited(key, 10, 60 * 60 * 1000)) return NextResponse.json({ error: "Rate limit exceeded" }, { status: 429 });
  ...
  if (isSupabaseConfigured()) {
    const supabase = getSupabaseServer();
    await supabase!.from("vibe_photos").insert({ spot_id: spotId, url });
  }
